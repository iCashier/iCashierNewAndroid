...
import com.icashier.app.activity.SignupActivity;
...
public class CountryPickerActivity extends AppCompatActivity {
       ...
       countryList.addAll(getAllCountries());
        ...
        searchEditText.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                for (int i = 0; i < COUNTRIES.length; i++) {
                    if ((new Locale(getResources().getConfiguration().locale.getLanguage(),
                            COUNTRIES[i].getCountryCode()).getDisplayCountry()).toLowerCase().startsWith(text.toLowerCase())||
                            (COUNTRIES[i].getCountryName()).toLowerCase().startsWith(text.toLowerCase())) {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent=new Intent(CountryPickerActivity.this,getCallingActivity().getClass());
                intent.putExtra("COUNTRY_CODE",countryList.get(position).getDileCode());
                setResult(5,intent);
                finish();
            }
        });
    ...
    public static int getFlagByCountryCode(String code){
        int flag=0;
        for(int i=0;i<COUNTRIES.length;i++){
            if(code.equals(COUNTRIES[i].dileCode)){
